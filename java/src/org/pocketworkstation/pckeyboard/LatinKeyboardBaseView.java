...
//                        + " dX=" + deltaX + " dy=" + deltaY + " min=" + travelMin);
    ...
    public void setKeyboard(Keyboard keyboard) {
        ...
        setKeys(mKeyDetector.setKeyboard(keyboard, -getPaddingLeft(),
                -getPaddingTop() + mVerticalCorrection));
        mKeyboardVerticalGap = (int)getResources().getDimension(R.dimen.key_bottom_gap);
        for (PointerTracker tracker : mPointerTrackers) {
            tracker.setKeyboard(getKeys(), mKeyHysteresisDistance);
        }
        ...
        if(mGestureHandler == null){
        	mGestureHandler = new GestureHandler(keyboard, mKeyDetector);
        } else{
        	mGestureHandler.setKeyboard(keyboard);
        }
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        mCanvas = canvas;
        mGestureHandler.drawPath(mCanvas);
...
//            }
    ...
    //TODO: Integrate with suggest, LatinIME
    public boolean onTouchEvent(MotionEvent me, boolean continuing) {
    	mCanvas = mGestureHandler.processMotionEvent(me, mCanvas);
    	//ArrayList<PointF> mPoints = mGestureHandler.getCurPoints();
    	
    	String swipeInput = mGestureHandler.getCurString();
    	mGestureHandler.clearCurString();
    	if(swipeInput.length() > 1){
    		mKeyboardActionListener.onText(swipeInput);
   			return true;
   		}
    	invalidate();
