...
public class Suggest implements Dictionary.WordCallback {
    ...
    private static boolean haveSufficientCommonality(String original, CharSequence suggestion) {
    ...
    public List<CharSequence> getSuggestions(View view, WordComposer wordComposer, 
        ...
        if (wordComposer.size() >= 1 && (mCorrectionMode == CORRECTION_FULL_BIGRAM
            ...
            if (!TextUtils.isEmpty(prevWordForBigram)) {
                ...
                sb.setLength(0);
                for(int j = 0; j < wordComposer.getTypedWord().length(); j++){
                	char currentChar = wordComposer.getTypedWord().charAt(j);
                    char currentCharUpper = Character.toUpperCase(currentChar);
                    int bigramSuggestionSize = mBigramSuggestions.size();
                    for (int i = 0; i < bigramSuggestionSize; i++) {
                        if (mBigramSuggestions.get(i).charAt(j) == currentChar
                                || mBigramSuggestions.get(i).charAt(j) == currentCharUpper) {
        ...
        removeDupes();
        Log.d(TAG, mSuggestions.size() + " suggestions from mSuggest from word " + mOriginalWord);
        return mSuggestions;
    }
