...
public class LatinIME extends InputMethodService implements ComposeSequencing,
	...
	private boolean mSwypeInput = false;
	...
	// Saved shift state when leaving alphabet mode, or when applying multitouch
	// shift
	...
	private SwypeDictionary mSwypeDict;
	...
	public void onCreate() {
		Log.i("PCKeyboard",
				"onCreate(), os.version=" + System.getProperty("os.version"));
		...
		mHeightPortrait = getHeight(prefs, PREF_HEIGHT_PORTRAIT,
		...
		LatinIME.sKeyboardSettings.hintMode = Integer.parseInt(prefs.getString(
				PREF_HINT_MODE, res.getString(R.string.default_hint_mode)));
		LatinIME.sKeyboardSettings.longpressTimeout = getPrefInt(prefs,
				PREF_LONGPRESS_TIMEOUT,
				res.getString(R.string.default_long_press_duration));
		LatinIME.sKeyboardSettings.renderMode = getPrefInt(prefs,
				PREF_RENDER_MODE, res.getString(R.string.default_render_mode));
		mSwipeUpAction = prefs.getString(PREF_SWIPE_UP,
				res.getString(R.string.default_swipe_up));
		mSwipeDownAction = prefs.getString(PREF_SWIPE_DOWN,
				res.getString(R.string.default_swipe_down));
		mSwipeLeftAction = prefs.getString(PREF_SWIPE_LEFT,
				res.getString(R.string.default_swipe_left));
		mSwipeRightAction = prefs.getString(PREF_SWIPE_RIGHT,
				res.getString(R.string.default_swipe_right));
		mVolUpAction = prefs.getString(PREF_VOL_UP,
				res.getString(R.string.default_vol_up));
		mVolDownAction = prefs.getString(PREF_VOL_DOWN,
				res.getString(R.string.default_vol_down));
	...
	private void updateKeyboardOptions() {
		...
		int screenHeightPercent = isPortrait ? mHeightPortrait
				: mHeightLandscape;
	...
	private void setNotification(boolean visible) {
		...
		if (visible && mNotificationReceiver == null) {
			...
			PendingIntent contentIntent = PendingIntent.getBroadcast(
					getApplicationContext(), 1, notificationIntent, 0);
	...
	public View onCreateInputView() {
		setCandidatesViewShown(false); // Workaround for "already has a parent"
										// when reconfiguring
	...
	public View onCreateCandidatesView() {
		// Log.i(TAG, "onCreateCandidatesView(), mCandidateViewContainer=" +
	...
	private void removeCandidateViewContainer() {
		// Log.i(TAG, "removeCandidateViewContainer(), mCandidateViewContainer="
		// + mCandidateViewContainer);
		if (mCandidateViewContainer != null) {
	...
	public void onStartInputView(EditorInfo attribute, boolean restarting) {
		...
		// + Integer.toHexString(attribute.inputType) + ", restarting=" +
		// restarting);
	...
	public void onFinishCandidatesView(boolean finishingInput) {
		// Log.i(TAG, "onFinishCandidatesView(), mCandidateViewContainer=" +
		// mCandidateViewContainer);
	...
	public boolean onEvaluateInputViewShown() {
		...
		// Log.i(TAG, "OnEvaluateInputViewShown, parent=" + parent + " + "
		// wanted=" + wanted);
		return wanted;
	}
	...
	public boolean isKeyboardVisible() {
		return (mKeyboardSwitcher != null
				&& mKeyboardSwitcher.getInputView() != null && mKeyboardSwitcher
	...
	private void postUpdateShiftKeyState() {
		// TODO(klausw): disabling, I have no idea what this is supposed to
		// accomplish.
		// //updateShiftKeyState(getCurrentInputEditorInfo());
		//
		// // FIXME: why the delay?
	...
	public void updateShiftKeyState(EditorInfo attr) {
		...
		if (ic != null && attr != null && mKeyboardSwitcher.isAlphabetMode()) {
			...
			// Log.i(TAG, "updateShiftKeyState isShifted=" + isShifted +
			// " isCaps=" + isCaps + " isMomentary=" +
			// mShiftKeyState.isMomentary() + " cursorCaps=" +
			// getCursorCapsMode(ic, attr));
			int newState = Keyboard.SHIFT_OFF;
			if (isShifted) {
				newState = (mSavedShiftState == Keyboard.SHIFT_LOCKED) ? Keyboard.SHIFT_CAPS
						: Keyboard.SHIFT_ON;
			} else if (isCaps) {
				newState = isCapsLock ? getCapsOrShiftLockState()
						: Keyboard.SHIFT_CAPS;
			}
			// Log.i(TAG, "updateShiftKeyState " + oldState + " -> " +
			// newState);
			mKeyboardSwitcher.setShiftState(newState);
		}
		if (ic != null) {
			// Clear modifiers other than shift, to avoid them getting stuck
			int states = KeyEvent.META_ALT_ON | KeyEvent.META_ALT_LEFT_ON
					| KeyEvent.META_ALT_RIGHT_ON | 0x8 // KeyEvent.META_FUNCTION_ON;
	...
	private boolean isConnectbot() {
		...
		return ((pkg.equalsIgnoreCase("org.connectbot")
				...
				|| pkg.equalsIgnoreCase("com.pslib.connectbot") || pkg
					.equalsIgnoreCase("sk.vx.connectbot")) && ei.inputType == 0); // FIXME
	}
	...
	private int getMetaState(boolean shifted) {
		...
		if (mModCtrl)
			meta |= 0x3000; // KeyEvent.META_CTRL_ON |
							// KeyEvent.META_CTRL_LEFT_ON;
		if (mModAlt)
			meta |= 0x30000; // KeyEvent.META_ALT_ON |
	...
	private void sendKeyDown(InputConnection ic, int key, int meta) {
		...
		if (ic != null)
			ic.sendKeyEvent(new KeyEvent(now, now, KeyEvent.ACTION_DOWN, key,
					0, meta));
	}
	...
	private void sendKeyUp(InputConnection ic, int key, int meta) {
		long now = System.currentTimeMillis();
		if (ic != null)
			ic.sendKeyEvent(new KeyEvent(now, now, KeyEvent.ACTION_UP, key, 0,
					meta));
	}
	...
	private void sendModifierKeysDown(boolean shifted) {
		...
		if (mModCtrl
				&& (!mCtrlKeyState.isMomentary() || delayChordingCtrlModifier())) {
			sendCtrlKey(ic, true, false);
		}
		if (mModAlt
				&& (!mAltKeyState.isMomentary() || delayChordingAltModifier())) {
	...
	private void sendSpecialKey(int code) {
		...
		// TODO(klausw): properly support xterm sequences for Ctrl/Alt
		// modifiers?
		// See
		// http://slackware.osuosl.org/slackware-12.0/source/l/ncurses/xterm.terminfo
		// and the output of "$ infocmp -1L". Support multiple sets, and
		// optional
		// true numpad keys?
		if (ESC_SEQUENCES == null) {
			...
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F1,
					KeyEvent.KEYCODE_1);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F2,
					KeyEvent.KEYCODE_2);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F3,
					KeyEvent.KEYCODE_3);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F4,
					KeyEvent.KEYCODE_4);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F5,
					KeyEvent.KEYCODE_5);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F6,
					KeyEvent.KEYCODE_6);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F7,
					KeyEvent.KEYCODE_7);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F8,
					KeyEvent.KEYCODE_8);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F9,
					KeyEvent.KEYCODE_9);
			CTRL_SEQUENCES.put(-LatinKeyboardView.KEYCODE_FKEY_F10,
					KeyEvent.KEYCODE_0);
	...
	public void sendModifiableKeyChar(char ch) {
		...
		if ((modShift || mModCtrl || mModAlt) && ch > 0 && ch < 127) {
			...
			// Restrict Shift modifier to ENTER and SPACE, supporting
			// Shift-Enter etc.
			// Note that most special keys such as DEL or cursor keys aren't
			// handled
			...
			if (combinedCode > 0) {
				...
				if (letter && !mModCtrl && !mModAlt) {
					// Try workaround for issue 179 where letters don't get
					// upcased
					ic.commitText(Character.toString(ch), 1);
					handleModifierKeysUp(false, false);
				} else {
		...
		if (ch >= '0' && ch <= '9') {
			// WIP
			InputConnection ic = getCurrentInputConnection();
			ic.clearMetaKeyStates(KeyEvent.META_SHIFT_ON | KeyEvent.META_ALT_ON
					| KeyEvent.META_SYM_ON);
			// EditorInfo ei = getCurrentInputEditorInfo();
		...
		// Default handling for anything else, including unmodified ENTER and
		// SPACE.
		sendKeyChar(ch);
	}
	...
	private boolean processMultiKey(int primaryCode) {
		if (mDeadAccentBuffer.composeBuffer.length() > 0) {
			// Log.i(TAG, "processMultiKey: pending DeadAccent, length=" +
			// mDeadAccentBuffer.composeBuffer.length());
	...
	// TODO for OpenSwype: put it through the suggest
	public void onText(CharSequence text) {
		...
		GestureHandler mGestureHandler = ((LatinKeyboardView) mKeyboardSwitcher
				.getInputView()).mGestureHandler;
		ArrayList<int[]> codesList = mGestureHandler.getCurCodes();

		for (int i = 0; i < text.length(); i++) {
			int primaryCode = (int) text.charAt(i);
			int[] codes = codesList.get(i);
			word.add(primaryCode, codes);
		}

		showSuggestionsFromSwype(word);
		// showSuggestions(word);

		/*
		 * InputConnection ic = getCurrentInputConnection(); if (ic == null)
		 * return; abortCorrection(false); ic.beginBatchEdit(); if (mPredicting)
		 * { commitTyped(ic, true); } maybeRemovePreviousPeriod(text);
		 * ic.commitText(text, 1); ic.endBatchEdit();
		 * updateShiftKeyState(getCurrentInputEditorInfo());
		 * mKeyboardSwitcher.onKey(0); // dummy key code. mJustRevertedSeparator
	...
	private void resetMultitouchShift() {
		int newState = Keyboard.SHIFT_OFF;
		if (mSavedShiftState == Keyboard.SHIFT_CAPS_LOCKED
	...
	private static int getCapsOrShiftLockState() {
		return sKeyboardSettings.capsLock ? Keyboard.SHIFT_CAPS_LOCKED
				: Keyboard.SHIFT_LOCKED;
	}

	// Rotate through shift states by successively pressing and releasing the
	// Shift key.
	...
	private void handleCharacter(int primaryCode, int[] keyCodes) {
		...
		if (isAlphabet(primaryCode) && isPredictionOn() && !mModCtrl
				&& !mModAlt && !isCursorTouchingWord()) {
			if (!mPredicting) {
				mPredicting = true;
				mComposing.setLength(0);
		...
		if (mModCtrl || mModAlt) {
			commitTyped(getCurrentInputConnection(), true); // sets
															// mPredicting=false
		}

		if (mPredicting) {
			if (isShiftCapsMode() && mKeyboardSwitcher.isAlphabetMode()
					&& mComposing.length() == 0) {
	...
	private void handleSeparator(int primaryCode) {
		...
		if (mPredicting) {
			...
			if (mAutoCorrectOn
				...
				// as "added an auto space" in autocomplete mode, but as
				// manually
	...
	private boolean isPredictionWanted() {
		return (mShowSuggestions || mSuggestionForceOn)
				&& !suggestionsDisabled();
	}
	...
	private void handleVoiceResults() {
		...
		boolean capitalizeFirstWord = preferCapitalization()
				|| (mKeyboardSwitcher.isAlphabetMode() && isShiftCapsMode());
		for (String c : mVoiceResults.candidates) {
			if (capitalizeFirstWord) {
				c = c.substring(0, 1)
						.toUpperCase(sKeyboardSettings.inputLocale)
	...
	private void showSuggestionsFromSwype(WordComposer composer) {
		CharSequence prevWord = EditingUtil.getPreviousWord(
				getCurrentInputConnection(), mWordSeparators);
		mSwypeDict.clearGuesses();
		long startTime = System.nanoTime();
		//mSwypeDict.getBigrams(composer, prevWord, mSuggest, null);
		mSwypeDict.getWordsFromSwype(composer);
		long endTime = System.nanoTime();
		long duration = endTime - startTime;
		accumTime += duration;
		reps++;
		Log.i(TAG, "Time to type "+ composer.getTypedWord() +": " + String.valueOf(duration));
		// }
		if(reps == 10){
			Log.i(TAG, RTAG + " Average speed in ns:" + String.valueOf(accumTime/reps));
			accumTime = reps = 0;
		}

		List<CharSequence> stringList = (List) mSwypeDict.getGuesses();
		if (stringList != null) {
			mSwypeInput = true;
			if (stringList.size() == 0) {
				stringList.add(composer.getTypedWord());
			}
		} else {
			stringList = new ArrayList<CharSequence>(1);
			stringList.add(composer.getTypedWord());
		}
		...
		CharSequence typedWord = composer.getTypedWord();
		...
		correctionAvailable &= !composer.isMostlyCaps();
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
		...
		if (sKeyboardSettings
				.hasFlag(GlobalKeyboardSettings.FLAG_PREF_NEED_RELOAD)) {
			needReload = true;
		}
		if (sKeyboardSettings
				.hasFlag(GlobalKeyboardSettings.FLAG_PREF_NEW_PUNC_LIST)) {
			initSuggestPuncList();
		}
		if (sKeyboardSettings
				.hasFlag(GlobalKeyboardSettings.FLAG_PREF_RECREATE_INPUT_VIEW)) {
			mKeyboardSwitcher.recreateInputView();
		}
		if (sKeyboardSettings
				.hasFlag(GlobalKeyboardSettings.FLAG_PREF_RESET_MODE_OVERRIDE)) {
			mKeyboardModeOverrideLandscape = 0;
			mKeyboardModeOverridePortrait = 0;
		}
		if (sKeyboardSettings
				.hasFlag(GlobalKeyboardSettings.FLAG_PREF_RESET_KEYBOARDS)) {
			toggleLanguage(true, true);
		}
		int unhandledFlags = sKeyboardSettings.unhandledFlags();
		if (unhandledFlags != GlobalKeyboardSettings.FLAG_PREF_NONE) {
			Log.w(TAG, "Not all flag settings handled, remaining="
					+ unhandledFlags);
		}
		...
		if (PREF_SELECTED_LANGUAGES.equals(key)) {
			...
			if (mReCorrectionEnabled) {
				...
				Toast.makeText(getApplicationContext(),
						res.getString(R.string.recorrect_warning),
			...
			mVolUpAction = sharedPreferences.getString(PREF_VOL_UP,
					res.getString(R.string.default_vol_up));
		} else if (PREF_VOL_DOWN.equals(key)) {
			mVolDownAction = sharedPreferences.getString(PREF_VOL_DOWN,
					res.getString(R.string.default_vol_down));
		...
		if (action == null || action.equals("") || action.equals("none")) {
			...
			if (LatinKeyboardView.DEBUG_AUTO_PLAY) {
				...
				if (!TextUtils.isEmpty(text)) {
					mKeyboardSwitcher.getInputView().startPlaying(
							text.toString());
				}
			}
			...
			if (VOICE_INSTALLED) {
				...
				Toast.makeText(
						...
						getResources().getString(
								R.string.voice_not_enabled_warning),
						Toast.LENGTH_LONG).show();
			}
		} else if (action.equals("full_mode")) {
			if (isPortrait()) {
				mKeyboardModeOverridePortrait = (mKeyboardModeOverridePortrait + 1)
						% mNumKeyboardModes;
			} else {
				mKeyboardModeOverrideLandscape = (mKeyboardModeOverrideLandscape + 1)
						% mNumKeyboardModes;
			}
			toggleLanguage(true, true);
	...
	private float getKeyClickVolume() {
		...
		// The volume calculations are poorly documented, this is the closest I
		// could
		...
		//
		// Note that the passed level value is a raw scalar. UI controls should
		// be scaled logarithmically:
		// the gain applied by audio framework ranges from -72dB to 0dB, so an
		// appropriate conversion
		// from linear UI input x to level is: x == 0 -> level = 0 0 < x <= R ->
		// level = 10^(72*(x-R)/20/R)

		int method = sKeyboardSettings.keyClickMethod; // See
														// click_method_values
														// in strings.xml
		if (method == 0)
			return FX_VOLUME;
		...
		if (method > 1) {
			// TODO(klausw): on some devices the media volume controls the click
			// volume?
			// If that's the case, try to set a relative target volume.
			int mediaMax = mAudioManager
					.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
			int mediaVol = mAudioManager
					.getStreamVolume(AudioManager.STREAM_MUSIC);
			// Log.i(TAG, "getKeyClickVolume relative, media vol=" + mediaVol +
			// "/" + mediaMax);
			...
			if (method == 2) {
				...
				targetVol = Math.min(targetVol / channelVol, 1.0f); // Cap at
																	// 1.0
			}
		}
		// Set absolute volume, treating the percentage as a logarithmic control
		float vol = (float) Math.pow(10.0, FX_VOLUME_RANGE_DB * (targetVol - 1)
				/ 20);
		// Log.i(TAG, "getKeyClickVolume absolute, target=" + targetVol +
		// " amp=" + vol);
		return vol;
	}
	...
	private void loadSettings() {
		...
		mVibrateLen = getPrefInt(sp, PREF_VIBRATE_LEN, getResources()
				.getString(R.string.vibrate_duration_ms));
		mSoundOn = sp.getBoolean(PREF_SOUND_ON, false);
		mPopupOn = sp.getBoolean(PREF_POPUP_ON,
				mResources.getBoolean(R.bool.default_popup_preview));
		mAutoCapPref = sp.getBoolean(PREF_AUTO_CAP,
				getResources().getBoolean(R.bool.default_auto_cap));
		...
		mLocaleSupportedForVoiceInput = voiceInputSupportedLocales
				...
				|| voiceInputSupportedLocales.contains(mInputLocale.substring(
						0, Math.min(2, mInputLocale.length())));
	...
	private boolean isSuggestedPunctuation(int code) {
		return sKeyboardSettings.suggestedPunctuation.contains(String
				.valueOf((char) code));
	}
	...
	static int getPrefInt(SharedPreferences prefs, String prefName, int defVal) {
		...
		// Log.i("PCKeyboard", "getPrefInt " + prefName + " = " + prefVal +
		// ", default " + defVal);
		return getIntFromString(prefVal, defVal);
	}

	static int getPrefInt(SharedPreferences prefs, String prefName,
			String defStr) {
